...
class InterProSeed(Seed):
    ...
    def get_seed(self, **kwargs):
        ...
        if return_pandas_assignment:
            return Seed._seed_to_pandas(combined.tocoo(), self.proteins, self.terms)
        return combined
    ...
    def process_output(self, **kwargs):
        ...
        for e, m in enumerate(self.methods):
            ...
            methods_df[m] = methods_df[m].merge(
                self.proteins, left_on="Protein", right_index=True
            )
            methods_df[m] = methods_df[m].merge(
                self.terms, left_on="GO ID", right_index=True
            )

            p_idx = methods_df[m]["protein idx"].values
            go_idx = methods_df[m]["term idx"].values

            self.all_methods += sparse.coo_matrix(
                (np.ones(len(methods_df[m])), (p_idx, go_idx)),
