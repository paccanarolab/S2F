...
class Diffuse(FancyApp.FancyApp):
    ...
    def run(self):
        ...
        self.read_labelling()
        diffusion = self.diffusion_method(self.graph, self.proteins, self.terms)
        diffusion.compute_kernel(**self.kernel_parameters)
        F = diffusion.diffuse(self.labelling, **self.kernel_parameters)
    ...
    def read_graph(self):
        ...
        graph_df = graph_df.merge(self.proteins, left_on='protein1', right_index=True)
        graph_df = graph_df.merge(self.proteins, left_on='protein2', right_index=True, suffixes=['1', '2'])
        p1_idx = graph_df['protein idx1'].values
        p2_idx = graph_df['protein idx2'].values

        A = sparse.coo_matrix((graph_df['score'], (p1_idx, p2_idx)),
                              shape=(len(self.proteins), len(self.proteins))).tocsr()
        self.graph = A.T + A
        self.graph = self.graph.tolil()  # this avoid the efficiency warning, although it can't be too bad
    ...
    def read_labelling(self):
        ...
        self.terms = pd.DataFrame(list(enumerate(sorted(labelling_df['goterm'].unique()))))
        self.terms.columns = ['term idx', 'term id']
        self.terms.set_index('term id', inplace=True)
        labelling_df = labelling_df.merge(self.proteins, left_on='protein', right_index=True)
        labelling_df = labelling_df.merge(self.terms, left_on='goterm', right_index=True)
        p_idx = labelling_df['protein idx'].values
        go_idx = labelling_df['term idx'].values
        self.labelling = sparse.coo_matrix((labelling_df['score'], (p_idx, go_idx)),
