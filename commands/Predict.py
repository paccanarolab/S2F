...
class Predict(FancyApp.FancyApp):
    ...
    def __init__(self, args):
        ...
        if os.path.exists(self.run_config):
            ...
            self.combined_graph = run_conf.get(
                "graphs", "combined_graph", fallback="compute"
            )
            self.graph_collection = run_conf.get(
                "graphs", "graph_collection", fallback="compute"
            )
            self.homology_graph = run_conf.get(
    ...
    def run(self):
        ...
        if self.load_homology_graph:
            ...
            sparse.save_npz(
                os.path.join(self.output_dir, "homology.npz"), graph_homology
            )
        else:
            graph_homology = None
        ...
        if self.load_combined_graph:
            combined_graph = self.combine_graphs(
                graph_collection, graph_homology, ip_seed
            )
        else:
            combined_graph = None
    ...
    def combine_interpro(self):
        if not os.path.exists(self.ip_seed_file):
            ...
            if self._is_structural_seed_file(self.interpro_output):
                self.tell(
                    "Detected structural assignments file; converting to seed matrix."
                )
                seed = self._build_structural_seed(self.interpro_output)
            else:
    ...
    def get_hmmer_blacklist(self):
        if self.hmmer_blacklist != "compute":
            return (
                pd.read_csv(self.hmmer_blacklist, header=None, names=["tax_id"])
                .tax_id.astype("str")
                .tolist()
            )
        return None
    ...
    def get_transfer_blacklist(self):
        if self.hmmer_blacklist != "compute":
            return (
                pd.read_csv(self.transfer_blacklist, header=None, names=["tax_id"])
                .tax_id.astype("str")
                .tolist()
            )
        return None
    ...
    def run_graph_homology(self):
        ...
        hom = homology.Homology(
            self.fasta, self.proteins, graphs_dir, self.alias, self.fasta_id_parser
        )
        hom.compute_graph()
    ...
    def _is_structural_seed_file(self, path):
    ...
    def _build_structural_seed(self, assignments_path):
        self.tell("Loading structural assignments from", assignments_path)
        try:
            assignments = pd.read_csv(assignments_path, sep="\t")
        except Exception as exc:
            raise RuntimeError(
                f"Unable to read structural assignments file {assignments_path}"
            ) from exc
        ...
        if not required_columns.issubset(assignments.columns):
            ...
            raise RuntimeError(
                f"Structural assignments file {assignments_path} is missing required columns {missing}"
            )
        ...
        seed_records = assignments[["Protein", "GO ID", "Score"]].drop_duplicates()
        seed_records = seed_records[seed_records["Score"] > 0]

        total_rows = len(seed_records)
        if total_rows == 0:
            self.warning(
                "Structural assignments file has no positive scores; returning empty seed."
            )
        ...
        if matched_proteins == 0:
            raise RuntimeError(
                "No proteins in the structural assignments match the FASTA-derived protein IDs."
            )
        protein_drop = total_rows - matched_proteins
        if protein_drop:
            self.warning(
                f"Dropped {protein_drop} structural assignments with proteins not present in FASTA index."
            )
        ...
        if matched_terms == 0:
            raise RuntimeError(
                "No GO terms in the structural assignments match the GO structure."
            )
        term_drop = matched_proteins - matched_terms
        if term_drop:
            self.warning(
                f"Dropped {term_drop} structural assignments with GO terms outside the GO structure."
            )
    ...
    def combine_graphs(self, graph_collection, graph_homology, ip_seed):
        if not os.path.exists(self.combined_graph_filename):
            comb = combination.Combination(
                self.proteins, graph_collection, graph_homology, ip_seed
            )
            comb.compute_graph()
