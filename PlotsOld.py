...
for i, organism in final_selection.sort_values(by='Tax ID').iterrows():
    ...
    interpro_file = os.path.join(
        S2F_DIR, f'seeds/interpro/{organism["Tax ID"]}.matrix')
    hmmer_file = os.path.join(
        S2F_DIR, f'seeds/hmmer/{organism["Tax ID"]}.matrix')
    interpro_diff_file = os.path.join(
        S2F_DIR, f'output/{organism["Tax ID"]}/interpro.diffusion')
    hmmer_diff_file = os.path.join(
        S2F_DIR, f'output/{organism["Tax ID"]}/hmmer.diffusion')
    s2f_file = os.path.join(
        S2F_DIR, f'output/{organism["Tax ID"]}/final.diffusion')
    ...
    if os.path.exists(org_dir + 'metrics_df.pkl'):
        ...
        if os.path.exists(org_dir):
            ...
            matrix_builder = BuildMatrices.BuildMatrices(
                interpro_file, interpro_diff_file, None,
                hmmer_file, hmmer_diff_file, None, s2f_file, goa,
                fasta, obo)
