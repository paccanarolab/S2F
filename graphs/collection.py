...
def compute_ortholog(fasta, db, string_fasta, string_db, out, out_dir):
    ...
    if not os.path.exists(outfile):
        blast_command = 'blastp -query {fasta} -db {blastdb} -out {out} -outfmt "6 std qlen" -max_target_seqs 1'

        # create temporary blast files
        out_1 = os.path.join(out_dir, out + 'forward')
        out_2 = os.path.join(out_dir, out + 'backward')
        subprocess.call(blast_command.format(fasta=fasta, blastdb=string_db, out=out_1), shell=True)
        subprocess.call(blast_command.format(fasta=string_fasta, blastdb=db, out=out_2), shell=True)
        ...
        orth['max evalue'] = np.max(orth[['query_evalue', 'target_evalue']], axis=1)
        orth['pos'] = np.sqrt(orth['query_pi'] * orth['target_pi'])
        orth.to_pickle(os.path.join(out_dir, out))
        # remove temporary files
        os.remove(out_1)
        os.remove(out_2)
    FancyApp.FancyApp.yell(ColourClass.bcolors.BOLD_CYAN, 'compute ortholog', 'Finished collecting orthologs:', out)
...
class Collection(Graph):
    ...
    def __init__(self,
                 ...
                 interesting_graphs=['neighborhood', 'experiments', 'coexpression', 'textmining', 'database']):
    ...
    def get_graph(self, **kwargs):
        ...
        for g in self.interesting_graphs:
            graph = self.collection[['query1', 'query2', g]].merge(self.proteins, left_on='query1', right_index=True)
            graph = graph.merge(self.proteins, left_on='query2', right_index=True, suffixes=['1', '2'])
    ...
    def should_be_processed(self, string_id):
        ...
        if string_id in self.blacklist:
            ...
            return False
        orthologs_filename = os.path.join(self.orthologs_dir, self.alias + '_AND_' + string_id)
    ...
    def process_graph(self, string_id, graph):
        ...
        # load ortholog file
        orthologs = pd.read_pickle(os.path.join(self.orthologs_dir, self.alias + '_AND_' + string_id))
        ...
        valid_orthologs = orthologs[
            ...
            (orthologs['target_perc'] >= self.perc) & (orthologs['query_perc'] >= self.perc) &
        ...
        valid_edges = valid_edges.merge(valid_orthologs[['query', 'target', 'max_evalue']],
                                        left_on='protein 1', right_on='target')
        valid_edges = valid_edges.merge(valid_orthologs[['query', 'target', 'max_evalue']],
                                        left_on='protein 2', right_on='target', suffixes=['1', '2'])
        valid_edges['max_evalue'] = valid_edges[['max_evalue1', 'max_evalue2']].max(axis=1)

        self.tell(valid_edges.shape[0], 'edges transferred from organism', string_id)

        drop_cols = ['protein 1', 'protein 2', 'target1', 'target2', 'max_evalue1', 'max_evalue2']
        if self.collection is None:
            self.collection = valid_edges.drop(drop_cols, axis=1)
        else:
            self.collection = self.collection.append(valid_edges.drop(drop_cols, axis=1), ignore_index=True)
    ...
    def compute_graph(self):
        ...
        if not os.path.exists(collection_file):
            ...
            if not (os.path.exists(self.fasta + '.phr') and os.path.exists(self.fasta + '.pin')):
                self.tell('Creating a blast database from the input fasta')
                subprocess.call('makeblastdb -in {fasta} -out {fasta} -dbtype prot'.format(fasta=self.fasta),
                                shell=True)
            ...
            for core_id in core_ids:
                ...
                params.append([self.fasta, self.db, fasta, fasta, out, self.orthologs_dir])
            ...
            for line in open(self.string_links):
                ...
                if first_line:
                    ...
                    # identify the index of the interesting graphs in the STRING file
                    ...
                    if org_id not in should_process.keys():
                        should_process[org_id] = self.should_be_processed(org_id)
                    ...
                    if current_organism == -1:
                        ...
                        # keeping only the relevant links (those with some information in at least one of the models)
                        condition = None
                        for g in self.interesting_graphs:
            ...
            # we make sure we don't miss possible links from the last organism in STRING
            ...
            Graph.assert_lexicographical_order(self.collection, p1='query1', p2='query2')
